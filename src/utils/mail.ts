...
export const sendVerificationOTP = async (token: string, profile: Profile) => {
  const { name, email } = profile

  const message = `Hi ${name}, welcome to Podify! There are so much thing that we do for verified users. Use the given OTP to verify your email.`
